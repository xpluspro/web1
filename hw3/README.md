...
本项目将原来的多页面静态书店重构为基于 React 和 React Router 的单页应用。现在书籍列表页、详情页、购物车、订单确认页、支付成功页、我的订单页和登录页都统一运行在同一个前端工程中，书籍数据集中管理，页面间跳转由路由完成，购物车状态和订单状态由 React 状态与 `localStorage` 共同维护。
...
- 使用 React Router 管理路由，核心路由包括 `/`、`/books/:slug`、`/cart`、`/order`、`/success`、`/myorder`、`/login`。
...
- 新增订单持久化逻辑 `src/lib/orderStorage.js`，支持订单记录、状态切换和我的订单页展示。
- 首页推荐区改造成自动轮播组件，支持左右切换、指示点跳转与首尾无缝循环。
- 抽取公共布局与复用组件，包括页头、页脚、书卡、价格筛选、数量步进器、订单摘要和轮播推荐组件。
...
- `src/lib/orderStorage.js`：订单读写、订单列表持久化和订单状态更新。
...
- `src/pages/MyOrdersPage.jsx`：订单中心页面，展示全部订单并支持按状态筛选。
- `src/components/HeroSection.jsx`：首页推荐轮播组件，展示推荐书籍并支持循环切换。
...
- 订单列表状态由 `App.jsx` 统一维护，变更后自动写入 `localStorage`。
- 在订单确认页，用户既可以直接支付生成“已支付”订单，也可以保存为“待支付”订单。
- “我的订单”页面支持 `全部 / 已支付 / 待支付` 三类查看方式，待支付订单可以继续支付并更新状态。
- 首页推荐区使用组件内部状态控制自动轮播，并支持无缝循环切换。
