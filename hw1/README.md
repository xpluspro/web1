...
### ⚙️ 逻辑交互 (JavaScript)
- `main.js`：**全站 UI 交互主干脚本**。汇集了所有页面的事件（如无缝轮播图调度逻辑、表单校验、所有点击事件诸如从分类页面与详情页发起“加入购物车”及各种收据打印功能的 `addEventListener`）。
- `cart.js`：**数据服务层脚本**。作为轻量级存储中心存在，专门封装读取/写入/修改/防重/清除浏览器的 localStorage 数据的方法，并将事件使用自定义抛出以驱动 UI 更新机制。
